...
<template>
  <label class="colorMode">
    <input type="checkbox" class="checkbox" @click="changeColor">
    <div class="back"></div>
...
.colorMode {
  ...
  transition: all .5s;
  margin: 1em;
  box-shadow: var(--secondary) 5px 5px 10px, var(--secondary) -5px -5px 10px;
}
...
.colorMode .checkbox:checked ~ .back {
  background-color: var(--primary);
}

.colorMode .checkbox:checked ~ .moon {
  transform: translate(100%) rotate(360deg);
  opacity: 0;
}

.colorMode .checkbox:checked ~ .sun {
